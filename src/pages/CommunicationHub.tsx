            ...
            <div className="space-y-2">
              {groups.map((group) => (
                <button
                  key={group.id}
                  onClick={() =>
                    navigate(`/dashboard/student/messages/${group.id}`)
                  }
          ...
          {activeGroup ? (
              ...
              <div className="p-4 md:p-6 border-b bg-background/50 flex justify-between items-center backdrop-blur-md shrink-0">
                ...
                <div className="flex gap-2 shrink-0">
                  <Button
                    ...
                    variant="secondary"
                    className="rounded-xl h-10 px-4 font-bold tracking-tight shadow-sm text-xs hidden sm:flex items-center gap-2"
                  >
                  ...
                  <Button
                    ...
                    variant="secondary"
                    size="icon"
                    className="rounded-xl h-10 w-10 shadow-sm sm:hidden"
                  >
                ...
                <div className="space-y-6 md:space-y-8 pb-4">
                  ...
                  {messages.map((msg) => {
                    ...
                    return (
                      <div
                        key={msg.id}
                        className={`flex gap-3 md:gap-4 ${isSelf ? "flex-row-reverse" : ""}`}
                      >
                        <Avatar className="w-10 h-10 border rounded-2xl shadow-sm shrink-0">
                          <AvatarFallback className="rounded-2xl font-bold bg-primary/5 text-primary">
                            {msg.senderName[0]}
                          </AvatarFallback>
                        </Avatar>
                        <div
                          className={`space-y-1.5 w-full flex flex-col ${isSelf ? "items-end" : "items-start"}`}
                        >
                          <div
                            className={`flex items-center gap-2 flex-wrap ${isSelf ? "flex-row-reverse" : ""}`}
                          >
                          ...
                          <div
                            className={`max-w-2xl shadow-sm border p-4 ${
                              isSelf
                          ...
                          >
                            {msg.imageUrl && (
                              <img
                                src={msg.imageUrl}
                                alt="attached"
                                className="rounded-xl mb-3 max-w-[200px] sm:max-w-xs object-cover shadow-sm"
                              />
                            )}
              ...
              <div className="p-4 md:p-6 bg-background/50 border-t backdrop-blur-md shrink-0">
                ...
                <div className="flex items-end gap-3 md:gap-4">
                  <div className="relative flex-1 bg-background rounded-[2rem] shadow-sm border border-muted/50 p-1.5">
                    ...
                    <div className="absolute right-2.5 top-1/2 -translate-y-1/2 flex gap-1">
                      <Popover>
                        ...
                        <PopoverContent
                          className="w-auto p-2 rounded-2xl"
                          align="end"
                          sideOffset={10}
                        >
                  ...
                  <Button
                    ...
                    className="h-[60px] w-[60px] md:h-14 md:w-14 rounded-full shadow-xl shadow-primary/25 shrink-0 bg-primary hover:bg-primary/90 transition-transform active:scale-95"
                  >
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground p-8 text-center">
              ...
              <p className="max-w-sm mb-6">
                Create a new group or ask a friend for their group link to start
                chatting!
              </p>
              <Button
                onClick={handleCreateGroup}
