...
export default function LoginPage() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (email === "student@campusflow.edu" && password === "student123") {
      const demoUser = DEMO_USERS["student@campusflow.edu"];
    ...
    if (email === "faculty@campusflow.edu" && password === "faculty123") {
      const demoUser = DEMO_USERS["faculty@campusflow.edu"];
      setDemoUser(demoUser);
    ...
    if (email === "admin@campusflow.edu" && password === "admin123") {
      const demoUser = DEMO_USERS["admin@campusflow.edu"];
      setDemoUser(demoUser);
  ...
  return (
    <div className="min-h-screen bg-muted/30 flex items-center justify-center p-4 selection:bg-primary/10">
      <Link
        to="/"
        className="absolute top-8 left-4 sm:left-8 flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors group"
      >
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground font-light mt-2">
            Log in to your specialized academic environment.
          </p>
        </div>
        ...
        <Card className="rounded-[2rem] border-none shadow-xl bg-background/80 backdrop-blur-xl">
              ...
              <div className="space-y-1 text-muted-foreground flex flex-col gap-1">
                <div className="flex justify-between border-b border-primary/10 pb-1">
                  ...
                  <span className="font-mono text-foreground font-medium">
                    student@campusflow.edu / student123
                  </span>
                </div>
                <div className="flex justify-between border-b border-primary/10 pb-1 pt-1">
                  <span>Faculty:</span>{" "}
                  <span className="font-mono text-foreground font-medium">
                    faculty@campusflow.edu / faculty123
                  </span>
                </div>
                <div className="flex justify-between pt-1">
                  <span>Admin:</span>{" "}
                  <span className="font-mono text-foreground font-medium">
                    admin@campusflow.edu / admin123
                  </span>
                </div>
              </div>
            </div>
          ...
          <CardContent>
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <Button
                type="submit"
                variant="secondary"
                className="w-full h-12 rounded-xl mt-4 font-semibold flex items-center gap-2 group"
              >
          ...
          <CardFooter className="flex flex-col items-center gap-4">
            <div className="w-full h-[1px] bg-border relative mt-2">
              <span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-background px-4 text-[10px] text-muted-foreground uppercase tracking-widest font-bold">
            ...
            >
              ...
              {isLoading
                ? "Signing in..."
                : "Institutional Single Sign-On (Google)"}
            </Button>
          </CardFooter>
        </Card>
